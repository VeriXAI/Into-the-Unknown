...
def load_EMNIST(data_train_model: DataSpec, data_test_model: DataSpec, data_train_monitor: DataSpec,
               data_test_monitor: DataSpec, data_run: DataSpec, adversarial_data_suffix=None):
    mnist_dataset_folder_path = DATA_PATH + "EMNIST"

    # URLs for the train image and label data
    url_train_image = 'train-images-idx3-ubyte.gz'
    url_train_labels = 'train-labels-idx1-ubyte.gz'
    num_train_samples = 112800

    # print("Downloading train data")
    # train = try_download(url_train_image, url_train_labels, num_train_samples)

    # URLs for the test image and label data
    url_test_image = 'test-images-idx3-ubyte.gz'
    url_test_labels = 'test-labels-idx1-ubyte.gz'
    num_test_samples = 18800
    ...
    if adversarial_data_suffix is not None:
        with open(mnist_dataset_folder_path + "/adversarial{}".format(adversarial_data_suffix), mode='rb') as file:
...
def loadData(src, cimg):
    if DOWNLOAD:
        src = "https://www.itl.nist.gov/iaui/vip/cs_links/EMNIST/" + src
        print('Downloading ' + src)
        gzfname, h = urlretrieve(src, './delete.me')
        print('Done.')
    else:
        gzfname = DATA_PATH + "EMNIST/{}".format(src)
