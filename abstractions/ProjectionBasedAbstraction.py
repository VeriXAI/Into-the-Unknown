...
class ProjectionBasedAbstraction(PartitionBasedAbstraction):
    def __init__(self, n: int, partition: list, abstractions, epsilon=0., epsilon_relative: bool = USE_EPSILON_RELATIVE,
                 dim: int = -1):
    ...
    def isknown(self, vector, distribution, skip_confidence=False, novelty_mode=False):
        ...
        for abstraction, block in zip(self.abstractions, self.partition):
            ...
            accepted, confidence_current = abstraction.isknown(projected_vector, distribution, skip_confidence=skip_confidence,
    ...
    def euclidean_mean_distance_absolute(self, vector):
        ...
        for abstraction, block in zip(self.abstractions, self.partition):
            projected_vector = [vector[i] for i in range(dim, dim + block)]
            distance_projection = abstraction.euclidean_mean_distance_absolute(projected_vector)
