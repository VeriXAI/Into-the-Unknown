...
class PointCollection(object):
    ...
    def set_distance_factor(self, new_factor):
        assert new_factor > 0, "Distance factor must be positive!"
        self._distance_factor = new_factor
    ...
    def euclidean_mean_distance(self, point, epsilon, epsilon_relative):
        ...
        radius = euclidean(closest_point, self.mean())
        if radius == 0.0:
    ...
    def center_distance(self, point, epsilon=None, epsilon_relative=None):
        # distance to set center
        return euclidean(point, self.center())

    def mean_distance(self, point, epsilon=None, epsilon_relative=None):
        # distance to cluster mean
        return euclidean(point, self.mean())
