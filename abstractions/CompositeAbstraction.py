...
class CompositeAbstraction(Abstraction):
    ...
    def isknown(self, vector, distribution, skip_confidence=False, novelty_mode=False):
        ...
        if COMPOSITE_ABSTRACTION_POLICY == 0:
            ...
            n_unknown = 0
            for abstraction in self.abstractions:
                result, confidence = abstraction.isknown(vector, distribution, skip_confidence=skip_confidence,
        ...
        # maximum
            ...
            for abstraction in self.abstractions:
                result, confidence = abstraction.isknown(vector, distribution)
                if result:
    ...
    def euclidean_mean_distance_absolute(self, vector):
        min_distance = inf
        for abstraction in self.abstractions:
            min_distance = min(min_distance, abstraction.euclidean_mean_distance_absolute(vector))
        return min_distance
