...
def test_alpha(model, data, history, alpha):
    ...
    b = n / (n - 1)
    a = -b
    confidences = model.predict(data.inputs())  # TODO CS: probably have to map the labels/classes
    predictions = [np.argmax(x) for x in confidences]  # NOTE: no mapping to data labels needed here
